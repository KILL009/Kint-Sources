...
namespace OpenNos.Import.Console
{
    public class ImportFactory
    {
        ...
        private readonly List<string[]> packetList = new List<string[]>();
        ...
        {
            var fileCardDat = $"{folder}\\Card.dat";
            var fileCardLang = $"{folder}\\_code_{ConfigurationManager.AppSettings["Language"]}_Card.txt";
        ...
        {
            var fileMapIdDat = $"{folder}\\MapIDData.dat";
            var fileMapIdLang = $"{folder}\\_code_{ConfigurationManager.AppSettings["Language"]}_MapIDData.txt";
            var folderMap = $"{folder}\\map";
        ...
        public void ImportNpcMonsterData()
        {
            foreach (string[] currentPacket in packetList.Where(o => o[0].Equals("e_info") && o[1].Equals("10")))
            {
                if (currentPacket.Length > 25)
                {
                    ...
                    var npcMonster = DAOFactory.NpcMonsterDAO.FirstOrDefault(s => s.NpcMonsterVNum == npcvnum);
                    if (npcMonster == null)
                    {
                        continue;
                    }
        ...
        public void ImportNpcMonsters()
        {
            ...
            var fileNpcId = $"{folder}\\monster.dat";
            var fileNpcLang = $"{folder}\\_code_{ConfigurationManager.AppSettings["Language"]}_monster.txt";
        ...
        {
            var filePacket = $"{folder}\\packet.txt";
        ...
        public void ImportRecipe()
        {
            try
            {
                ...
                short item = 0;
                RecipeDTO recipe;
                foreach (string[] currentPacket in packetList.Where(o => o[0].Equals("n_run") || o[0].Equals("pdtse") || o[0].Equals("m_list")))
                {
                    ...
                    if (currentPacket.Length > 1 && currentPacket[0] == "m_list" && (currentPacket[1] == "2" || currentPacket[1] == "4"))
                    {
                        for (int i = 2; i < currentPacket.Length - 1; i++)
                        {
                            ...
                            var mapnpcid1 = mapnpcid;
                            var recipe1 = recipe;
                            var test = DAOFactory.RecipeDAO.InsertOrUpdate(ref recipe);
                    ...
                    for (int i = 3; i < currentPacket.Length - 1; i += 2)
                    {
                        var item1 = item;
                        var mapnpcid1 = mapnpcid;
                        var rec = DAOFactory.RecipeDAO.FirstOrDefault(s => s.MapNpcId.Equals(mapnpcid1) && s.ItemVNum == item1);
                        if (rec != null)
                        {
                            rec.Amount = byte.Parse(currentPacket[2]);
                            DAOFactory.RecipeDAO.InsertOrUpdate(ref rec);
                            var recipedto = DAOFactory.RecipeDAO.FirstOrDefault(s => s.MapNpcId.Equals(mapnpcid1) && s.ItemVNum == item1);
                            if (recipedto == null)
                            {
                                continue;
                            }

                            var recipeId = recipedto.RecipeId;
                    ...
                    if (currentPacket.Length > 1 && currentPacket[0] == "m_list" && (currentPacket[1] == "3" || currentPacket[1] == "5"))
                    {
                        for (int i = 3; i < currentPacket.Length - 1; i += 2)
                        {
                            var rec = DAOFactory.RecipeDAO.FirstOrDefault(s => s.MapNpcId.Equals(mapnpcid) && s.ItemVNum == item);
                            if (rec != null)
                            {
                                rec.Amount = byte.Parse(currentPacket[2]);
                                DAOFactory.RecipeDAO.InsertOrUpdate(ref rec);
                                ...
                                if (DAOFactory.RecipeItemDAO.FirstOrDefault(s => s.RecipeId.Equals(rec.RecipeId) && s.ItemVNum.Equals(recipeitem.ItemVNum)) == null)
                                {
                                    DAOFactory.RecipeItemDAO.InsertOrUpdate(ref recipeitem);
                                }
                            }
            ...
            {
                Logger.Log.Info("Error on Recipe!");
            }
        }
        ...
        public void ImportScriptedInstances()
        {
            ...
            foreach (string[] currentPacket in packetList.Where(o => o[0].Equals("at") || o[0].Equals("wp") || o[0].Equals("gp") || o[0].Equals("rbr")))
            {
                if (currentPacket.Length > 5 && currentPacket[0] == "at")
                {
                    map = short.Parse(currentPacket[2]);
        ...
        public void ImportShopItems()
        {
            ...
            var itemCounter = 0;
            byte type = 0;
            foreach (string[] currentPacket in packetList.Where(o => o[0].Equals("n_inv") || o[0].Equals("shopping")))
            {
        ...
        public void ImportShops()
        {
            ...
            foreach (string[] currentPacket in packetList.Where(o => o.Length > 6 && o[0].Equals("shop") && o[1].Equals("2")))
            {
                var npcid = short.Parse(currentPacket[2]);
        ...
        {
            var fileSkillId = $"{folder}\\Skill.dat";
            var fileSkillLang = $"{folder}\\_code_{ConfigurationManager.AppSettings["Language"]}_Skill.txt";
        ...
        public void ImportTeleporters()
        {
            ...
            foreach (string[] currentPacket in packetList.Where(o => o[0].Equals("at") || o[0].Equals("n_run") && (o[1].Equals("16") || o[1].Equals("26") || o[1].Equals("45") || o[1].Equals("301") || o[1].Equals("132") || o[1].Equals("5002") || o[1].Equals("5012"))))
            {
                if (currentPacket.Length > 4 && currentPacket[0] == "n_run")
                {
                    var npcid = int.Parse(currentPacket[4]);
        ...
        public void LoadMaps() => maps = DAOFactory.MapDAO.LoadAll().ToList();

        internal void ImportItems()
        {
            var fileId = $"{folder}\\Item.dat";
            var fileLang = $"{folder}\\_code_{ConfigurationManager.AppSettings["Language"]}_Item.txt";
                            ...
                            case ItemType.Jewelery:
                                switch (item.EquipmentSlot)
                                {
                                    case EquipmentType.Amulet:
                                        ...
                                        {
                                            // (item.VNum > 8104 && item.VNum < 8115) <= disaled for
                                            // now because doesn't work!
                                            item.ItemValidTime = 10800;
                                        }
                                        else
                                        {
