        ...
        public void RunEvent(EventContainer evt, ClientSession session = null, MapMonster monster = null)
        {
            ...
            switch (evt.EventActionType)
            {
                ...
                case EventActionType.SCRIPTEND:
                    switch (evt.MapInstance.MapInstanceType)
                    {
                        case MapInstanceType.TimeSpaceInstance:
                            ...
                            var client = evt.MapInstance.Sessions.FirstOrDefault();
                            if (client != null)
                            {
                                var mapInstanceId = ServerManager.Instance.GetBaseMapInstanceIdByMapId(client.Character.MapId);
                                var map = ServerManager.Instance.GetMapInstance(mapInstanceId);
                                var si = map.ScriptedInstances.FirstOrDefault(s => s.PositionX == client.Character.MapX && s.PositionY == client.Character.MapY);
                                byte penalty = 0;
                        ...
                        case MapInstanceType.RaidInstance:
                            ...
                            if (client != null)
                            {
                                var grp = client.Character?.Group;
                                if (grp == null)
                                {
                                    return;
                                }
                ...
                case EventActionType.CHANGEPORTALTYPE:
                    ...
                    var portal = evt.MapInstance.Portals.FirstOrDefault(s => s.PortalId == param.Item1);
                    if (portal != null)
                    {
                ...
                case EventActionType.SPAWNONLASTENTRY:
                    ...
                    var hornTarget = lastincharacter?.CharacterId ?? -1;
