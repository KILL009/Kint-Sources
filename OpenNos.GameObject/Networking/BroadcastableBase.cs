...
namespace OpenNos.GameObject
{
    public abstract class BroadcastableBase : IDisposable
    {
        ...
        /// </summary>
        private readonly ConcurrentDictionary<long, ClientSession> sessions;
        ...
        public IEnumerable<ClientSession> Sessions
        {
            get
            {
                return sessions.Select(s => s.Value).Where(s => s.HasSelectedCharacter && !s.IsDisposing && s.IsConnected);
            }
        }
        ...
        public void Broadcast(string packet) => Broadcast(null, packet);
        ...
        {
            return sessions.ContainsKey(characterId) ? sessions[characterId] : null;
        }
        ...
        public void UnregisterSession(long characterId)
        {
            ...
            if (!sessions.TryRemove(characterId, out ClientSession session))
            {
                return;
            }
        ...
        private void SpreadBroadcastpacket(BroadcastPacket sentPacket)
        {
            ...
            switch (sentPacket.Receiver)
            {
                ...
                case ReceiverType.OnlySomeone:
                    ...
                    {
                        var targetSession = Sessions.SingleOrDefault(s => s.Character.CharacterId == sentPacket.SomeonesCharacterId || s.Character.Name == sentPacket.SomeonesCharacterName);
