...
namespace OpenNos.GameObject
{
    public class ServerManager : BroadcastableBase, IDisposable
    {
        ...
        private List<DropDTO> generalDrops = new List<DropDTO>();
        private bool inRelationRefreshMode;
        private long lastGroupId;

        private ConcurrentDictionary<short, List<MapNpc>> mapNpcs;

        private ConcurrentDictionary<short, List<DropDTO>> monsterDrops;

        private ConcurrentDictionary<short, List<NpcMonsterSkill>> monsterSkills;

        private ConcurrentDictionary<int, List<Recipe>> recipes;

        private ConcurrentDictionary<int, List<ShopItemDTO>> shopItems;

        private ConcurrentDictionary<int, Shop> shops;

        private ConcurrentDictionary<int, List<ShopSkillDTO>> shopSkills;

        private ConcurrentDictionary<int, List<TeleporterDTO>> teleporters;
        ...
        public List<Mall> MallItems { get; set; }
            ...
            try
            {
                ...
                if (Groups != null)
                {
                    Parallel.ForEach(Groups, group =>
                    {
                        ...
                        {
                            var chara = Sessions.FirstOrDefault(s =>
        ...
        {
            var map = maps.FirstOrDefault(m => m.MapId.Equals(mapId));
            ...
            if (mapInstance.MapInstanceType != MapInstanceType.ArenaInstance)
            {
        ...
        {
            return monsterDrops.ContainsKey(monsterVNum) ? generalDrops.Concat(monsterDrops[monsterVNum]).ToList() : new List<DropDTO>();
        }
        ...
        public Item GetItem(short vnum) => items.FirstOrDefault(m => m.VNum.Equals(vnum));
        ...
        {
            return recipes.ContainsKey(mapNpcId) ? recipes[mapNpcId] : new List<Recipe>();
        }
        ...
        public T GetUserMethod<T>(long characterId, string methodName)
        {
            ...
            var method = session.Character.GetType().GetMethod(methodName);
        ...
        public void GroupLeave(ClientSession session)
        {
            ...
            var grp = Instance.Groups.FirstOrDefault(s => s.IsMemberOfGroup(session.Character.CharacterId));
            if (grp == null)
            {
                return;
            }
        ...
        public void Initialize()
        {
            ...
            monsterDrops = new ConcurrentDictionary<short, List<DropDTO>>();
            Parallel.ForEach(DAOFactory.DropDAO.LoadAll().GroupBy(d => d.MonsterVNum), monsterDropGrouping =>
            {
                ...
                {
                    monsterDrops[monsterDropGrouping.Key.Value] = monsterDropGrouping.OrderBy(d => d.DropChance).ToList();
                }
                else
                {
                    generalDrops.AddRange(monsterDropGrouping.ToList());
                }
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("DROPS_LOADED"), monsterDrops.Sum(i => i.Value.Count)));

            // initialize monsterskills
            monsterSkills = new ConcurrentDictionary<short, List<NpcMonsterSkill>>();
            Parallel.ForEach(DAOFactory.NpcMonsterSkillDAO.LoadAll().GroupBy(n => n.NpcMonsterVNum), monsterSkillGrouping =>
            {
                monsterSkills[monsterSkillGrouping.Key] = monsterSkillGrouping.Select(n => n as NpcMonsterSkill).ToList();
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("MONSTERSKILLS_LOADED"), monsterSkills.Sum(i => i.Value.Count)));

            // initialize Bazaar
            LoadBazaar();
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("BAZAR_LOADED"), monsterSkills.Sum(i => i.Value.Count)));
            ...
            Parallel.ForEach(DAOFactory.NpcMonsterDAO.LoadAll(), npcMonster =>
            {
                ...
                var monster = npcMonsters[npcMonster.NpcMonsterVNum];
                if (monster != null)
                {
            ...
            // intialize recipes
            recipes = new ConcurrentDictionary<int, List<Recipe>>();
            Parallel.ForEach(DAOFactory.RecipeDAO.LoadAll().GroupBy(r => r.MapNpcId), recipeGrouping =>
            {
                recipes[recipeGrouping.Key] = recipeGrouping.Select(r => r as Recipe).ToList();
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("RECIPES_LOADED"), recipes.Sum(i => i.Value.Count)));

            // initialize shopitems
            shopItems = new ConcurrentDictionary<int, List<ShopItemDTO>>();
            Parallel.ForEach(DAOFactory.ShopItemDAO.LoadAll().GroupBy(s => s.ShopId), shopItemGrouping =>
            {
                shopItems[shopItemGrouping.Key] = shopItemGrouping.ToList();
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("SHOPITEMS_LOADED"), shopItems.Sum(i => i.Value.Count)));

            // initialize shopskills
            shopSkills = new ConcurrentDictionary<int, List<ShopSkillDTO>>();
            Parallel.ForEach(DAOFactory.ShopSkillDAO.LoadAll().GroupBy(s => s.ShopId), shopSkillGrouping =>
            {
                shopSkills[shopSkillGrouping.Key] = shopSkillGrouping.ToList();
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("SHOPSKILLS_LOADED"), shopSkills.Sum(i => i.Value.Count)));

            // initialize shops
            shops = new ConcurrentDictionary<int, Shop>();
            Parallel.ForEach(DAOFactory.ShopDAO.LoadAll(), shopGrouping =>
            {
                shops[shopGrouping.MapNpcId] = (Shop)shopGrouping;
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("SHOPS_LOADED"), shops.Count));

            // initialize teleporters
            teleporters = new ConcurrentDictionary<int, List<TeleporterDTO>>();
            Parallel.ForEach(DAOFactory.TeleporterDAO.LoadAll().GroupBy(t => t.MapNpcId), teleporterGrouping =>
            {
                teleporters[teleporterGrouping.Key] = teleporterGrouping.Select(t => t).ToList();
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("TELEPORTERS_LOADED"), teleporters.Sum(i => i.Value.Count)));
            ...
            skills.AddRange(_skill.Select(s => s.Value));
            ...
            // intialize mapnpcs
            mapNpcs = new ConcurrentDictionary<short, List<MapNpc>>();
            Parallel.ForEach(DAOFactory.MapNpcDAO.LoadAll().GroupBy(t => t.MapId), mapNpcGrouping =>
            {
                mapNpcs[mapNpcGrouping.Key] = mapNpcGrouping.Select(t => t as MapNpc).ToList();
            });
            Logger.Log.Info(string.Format(Language.Instance.GetMessageFromKey("MAPNPCS_LOADED"), mapNpcs.Sum(i => i.Value.Count)));
            ...
            try
            {
                ...
                {
                    var guid = Guid.NewGuid();
                    var mapinfo = new Map(map.MapId, map.Data)
                    {
                        Music = map.Music
                    };
                    _mapList[map.MapId] = mapinfo;
                    var newMap = new MapInstance(mapinfo, guid, map.ShopAllowed, MapInstanceType.BaseMapInstance, new InstanceBag());
                    ...
                    Parallel.ForEach(newMap.Monsters, mapMonster =>
                    {
                        ...
                        mapMonster.Monster.BCards.ForEach(s =>
                        {
                            if (s.Type == 26 /* OnDeath Summon */)
                            {
                                ConcurrentBag<MonsterToSummon> summonParameters = new ConcurrentBag<MonsterToSummon>();

                                for (int j = 0; j < s.FirstData; j++)
                                {
                                    short x, y;
                                    if (s.SubType == 11)
                                    {
                                        x = (short)(j + mapMonster.MapX);
                                        y = mapMonster.MapY;
                                    }
                                    else
                                    {
                                        x = (short)(RandomNumber(-3, 3) + mapMonster.MapX);
                                        y = (short)(RandomNumber(-3, 3) + mapMonster.MapY);
                                    }

                                    summonParameters.Add(new MonsterToSummon((short)s.SecondData, new MapCell { X = x, Y = y }, -1, true));
                                }

                                if (mapMonster.OnDeathEvents.All(x => x.EventActionType != EventActionType.SPAWNMONSTERS))
                                {
                ...
                maps.AddRange(_mapList.Select(s => s.Value));
                if (i != 0)
                {
                ...
                LoadNosmall();
                Logger.Log.Info($"{MallItems.Count} Nosmall Items loaded!");
                ...
                if (DAOFactory.MapDAO.Where(s => s.MapId == 148).Any())
                {
                    ...
                    Act4ShipDemon = GenerateMapInstance(148, MapInstanceType.NormalInstance, null);
                ...
                {
                    var act4Map = GenerateMapInstance(m.MapId, MapInstanceType.Act4Instance, new InstanceBag());
                    ...
                    {
                        var portal = act4Map.Portals.FirstOrDefault(s => s.DestinationMapId == 153);
                        if (portal != null)
                        {
                            portal.SourceX = 140;
                            portal.SourceY = 186;
                ...
                foreach (MapInstance m in Act4Maps)
                {
                    ...
                    {
                        var mapInstance = Act4Maps.FirstOrDefault(s => s.Map.MapId == portal.DestinationMapId);
                        if (mapInstance != null)
                        {
            ...
            var serverIdentification = Guid.NewGuid();
        ...
        {
            var session = Sessions.FirstOrDefault(s => s.Character != null && s.Character.Name.Equals(characterName));
            session?.Disconnect();
        }
        ...
        public void ReviveFirstPosition(long characterId)
        {
            ...
            switch (session.CurrentMapInstance.MapInstanceType)
            {
                ...
                case MapInstanceType.Act4Instance:
                    ...
                    var citadel = Instance.Act4Maps.FirstOrDefault(s => s.Map.MapId == (session.Character.Faction == FactionType.Angel ? 130 : 131));
                    if (citadel != null)
                    {
        ...
        {
            var message = string.Format(Language.Instance.GetMessageFromKey("SHUTDOWN_MIN"), 5);
        ...
        public void UpdateGroup(long charId)
        {
            try
            {
                var myGroup = Groups?.FirstOrDefault(s => s.IsMemberOfGroup(charId));
                if (myGroup == null)
                {
                    return;
                }
        ...
        {
            return monsterSkills.ContainsKey(npcMonsterVNum) ? monsterSkills[npcMonsterVNum] : new List<NpcMonsterSkill>();
        }

        internal Shop GetShopByMapNpcId(int mapNpcId)
        {
            return shops.ContainsKey(mapNpcId) ? shops[mapNpcId] : null;
        }

        internal List<ShopItemDTO> GetShopItemsByShopId(int shopId)
        {
            return shopItems.ContainsKey(shopId) ? shopItems[shopId] : new List<ShopItemDTO>();
        }

        internal List<ShopSkillDTO> GetShopSkillsByShopId(int shopId)
        {
            return shopSkills.ContainsKey(shopId) ? shopSkills[shopId] : new List<ShopSkillDTO>();
        }

        internal List<TeleporterDTO> GetTeleportersByNpcVNum(short npcMonsterVNum)
        {
            if (teleporters != null && teleporters.ContainsKey(npcMonsterVNum))
            {
                return teleporters[npcMonsterVNum];
            }
        ...
        private void Act4Process()
        {
            ...
            Act4RaidType CreateRaid(FactionType faction)
            {
                ...
                return (Act4RaidType)random.Value.Next(0, 5);
            }
        ...
        private void LoadBazaar()
        {
            ...
            foreach (BazaarItemDTO bz in DAOFactory.BazaarItemDAO.LoadAll())
            {
                ...
                var chara = DAOFactory.CharacterDAO.FirstOrDefault(s => s.CharacterId.Equals(bz.SellerId));
                if (chara != null)
                {
        ...
        private void LoadNosmall()
        {
            MallItems = new List<Mall>();
            foreach (MallDTO mall in DAOFactory.MallDAO.LoadAll())
            {
                MallItems.Add(mall);
            }
        }
        ...
        private void LoadFamilies()
        {
            ...
            {
                var family = Family.FromDTO(familyDTO);
        ...
        private void OnBazaarRefresh(object sender, EventArgs e)
        {
            ...
            var bzdto = DAOFactory.BazaarItemDAO.FirstOrDefault(s => s.BazaarItemId.Equals(bazaarId));
            var bzlink = BazaarList.FirstOrDefault(s => s.BazaarItem.BazaarItemId == bazaarId);
            lock (BazaarList)
            {
                if (bzdto != null)
                {
                    var chara = DAOFactory.CharacterDAO.FirstOrDefault(s => s.CharacterId.Equals(bzdto.SellerId));
                    if (bzlink != null)
                    {
        ...
        private void OnFamilyRefresh(object sender, EventArgs e)
        {
            ...
            var famdto = DAOFactory.FamilyDAO.FirstOrDefault(s => s.FamilyId.Equals(familyId));
            var fam = FamilyList.FirstOrDefault(s => s.FamilyId == familyId);
            lock (FamilyList)
            {
                if (famdto != null)
                {
                    if (fam != null)
                    {
                        var lod = fam.LandOfDeath;
                        FamilyList.Remove(fam);
                        fam = Family.FromDTO(famdto);
                        ...
                        foreach (FamilyCharacterDTO famchar in DAOFactory.FamilyCharacterDAO.Where(s => s.FamilyId.Equals(fam.FamilyId)))
                            fam.FamilyCharacters.Add(FamilyCharacter.FromDTO(famchar));

                        var familyLeader = fam.FamilyCharacters.FirstOrDefault(s => s.Authority == FamilyAuthority.Head);
                    ...
                    {
                        var fami = Family.FromDTO(famdto);
                        fami.FamilyCharacters = new List<FamilyCharacter>();
                        foreach (FamilyCharacterDTO famchar in DAOFactory.FamilyCharacterDAO.Where(s => s.FamilyId.Equals(fam.FamilyId)))
                            fami.FamilyCharacters.Add(FamilyCharacter.FromDTO(famchar));

                        var familyLeader = fami.FamilyCharacters.FirstOrDefault(s => s.Authority == FamilyAuthority.Head);
                        if (familyLeader != null)
                        {
        ...
        private void OnMailSent(object sender, EventArgs e)
        {
            ...
            var targetSession = Sessions.SingleOrDefault(s => s.Character.CharacterId == message.ReceiverId);
        ...
        private void OnMessageSentToCharacter(object sender, EventArgs e)
        {
            ...
            var targetSession = Sessions.SingleOrDefault(s => s.Character.CharacterId == message.DestinationCharacterId);
            switch (message.Type)
            {
        ...
        private void OnPenaltyLogRefresh(object sender, EventArgs e)
        {
            ...
            var reldto = DAOFactory.PenaltyLogDAO.FirstOrDefault(s => s.PenaltyLogId == relId);
            var rel = PenaltyLogs.FirstOrDefault(s => s.PenaltyLogId == relId);
            if (reldto != null)
            {
                if (rel != null)
                {
        ...
        private void OnRelationRefresh(object sender, EventArgs e)
        {
            ...
            {
                var reldto = DAOFactory.CharacterRelationDAO.FirstOrDefault(s => s.CharacterRelationId == relId);
                var rel = CharacterRelations.FirstOrDefault(s => s.CharacterRelationId == relId);
                if (reldto != null)
                {
                    if (rel != null)
                    {
        ...
        private void OnSessionKicked(object sender, EventArgs e)
        {
            ...
            var targetSession = Sessions.FirstOrDefault(s => (!kickedSession.Item1.HasValue || s.SessionId == kickedSession.Item1.Value)
