...
namespace OpenNos.GameObject
{
    public class WearableInstance : ItemInstance, IWearableInstance
    {
        ...
        public string GenerateEInfo()
        {
            ...
            var time = ItemDeleteTime != null ? (long)(test - DateTime.Now).TotalSeconds : 0;
            ...
            switch (itemType)
            {
                ...
                case ItemType.Box:
                    ...
                    switch (subtype)
                    {
                        ...
                        case 5:
                            var fairyitem = ServerManager.Instance.GetItem(specialist.HoldingVNum);
        ...
        public override void Initialize() => random = new Random();
            ...
            bool isCommand = false)
        {
            ...
            const byte CELLA = 5;
            const int CELLA_VNUM = 1014;
            const int SCROLL_VNUM = 1218;
            double rnd;
            ...
            if (session != null)
            {
                switch (mode)
                {
                    ...
                    case RarifyMode.Success:
                        ...
                        Rare += 1;
                        SetRarityPoint();
                        var inventory = session?.Character.Inventory.GetItemInstanceById(Id);
                        if (inventory != null)
                        {
                    ...
                    case RarifyMode.Reduced:
                        ...
                        if (session.Character.Inventory.CountItem(CELLA_VNUM) < CELLA * REDUCEDPRICEFACTOR)
                        {
                            return;
                        }

                        session.Character.Inventory.RemoveItemAmount(CELLA_VNUM, (int)(CELLA * REDUCEDPRICEFACTOR));
                    ...
                    case RarifyMode.Normal:
                        ...
                        if (session.Character.Inventory.CountItem(CELLA_VNUM) < CELLA)
                        {
                            return;
                        }

                        if (protection == RarifyProtection.Scroll && !isCommand &&
                            session.Character.Inventory.CountItem(SCROLL_VNUM) < 1)
                        {
                            return;
                        }
                        ...
                        if (protection == RarifyProtection.Scroll && !isCommand)
                        {
                            session.Character.Inventory.RemoveItemAmount(SCROLL_VNUM);
                            session.SendPacket("shop_end 2");
                        }

                        session.Character.Gold -= GOLDPRICE;
                        session.Character.Inventory.RemoveItemAmount(CELLA_VNUM, CELLA);
                        session.SendPacket(session.Character.GenerateGold());
                        break;
            ...
            var inventoryb = session.Character.Inventory.GetItemInstanceById(Id);
            if (inventoryb != null)
            {
        ...
        public void SetRarityPoint()
        {
            switch (Item.EquipmentSlot)
            {
                ...
                case EquipmentType.SecondaryWeapon:
                    {
                        var point = CharacterHelper.Instance.RarityPoint(Rare, Item.IsHeroic ? (short)(95 + Item.LevelMinimum) : Item.LevelMinimum);
                        Concentrate = 0;
                        HitRate = 0;
                        DamageMinimum = 0;
                        DamageMaximum = 0;
                        if (Rare >= 0)
                        {
                            ...
                            {
                                var rndn = ServerManager.Instance.RandomNumber(0, 3);
                                if (rndn == 0)
                                {
                                    Concentrate++;
                                    HitRate++;
        ...
        public void Sum(ClientSession session, WearableInstance itemToSum)
        {
            ...
            if (session.Character.Inventory.CountItem(SAND_VNUM) < sand[Upgrade])
            {
                return;
            }

            session.Character.Inventory.RemoveItemAmount(SAND_VNUM, (byte)sand[Upgrade]);
        ...
        public void UpgradeItem(ClientSession session, UpgradeMode mode, UpgradeProtection protection, bool isCommand = false)
        {
            ...
            const short CELLA_VNUM = 1014;
            const short GEM_VNUM = 1015;
            const short GEM_FULL_VNUM = 1016;
            const double REDUCEDPRICEFACTOR = 0.5;
            const short NORMAL_SCROLL_VNUM = 1218;
            const short GOLD_SCROLL_VNUM = 5369;
            ...
            switch (mode)
            {
                ...
                case UpgradeMode.Reduced:
                    ...
                    if (session.Character.Inventory.CountItem(CELLA_VNUM) < cella[Upgrade] * REDUCEDPRICEFACTOR)
                    {
                        session.SendPacket(session.Character.GenerateSay(
                            string.Format(Language.Instance.GetMessageFromKey("NOT_ENOUGH_ITEMS"), ServerManager.Instance.GetItem(CELLA_VNUM).Name, cella[Upgrade] * REDUCEDPRICEFACTOR), 10));
                        return;
                    }

                    if (protection == UpgradeProtection.Protected && !isCommand && session.Character.Inventory.CountItem(GOLD_SCROLL_VNUM) < 1)
                    {
                        session.SendPacket(session.Character.GenerateSay(
                            string.Format(Language.Instance.GetMessageFromKey("NOT_ENOUGH_ITEMS"), ServerManager.Instance.GetItem(GOLD_SCROLL_VNUM).Name, cella[Upgrade] * REDUCEDPRICEFACTOR), 10));
                        return;
                    }

                    if (Upgrade < 5)
                    {
                        if (session.Character.Inventory.CountItem(GEM_VNUM) < gem[Upgrade])
                        {
                            session.SendPacket(session.Character.GenerateSay(
                                string.Format(Language.Instance.GetMessageFromKey("NOT_ENOUGH_ITEMS"), ServerManager.Instance.GetItem(GEM_VNUM).Name, gem[Upgrade]), 10));
                            return;
                        }

                        session.Character.Inventory.RemoveItemAmount(GEM_VNUM, gem[Upgrade]);
                    }
                    else
                    {
                        if (session.Character.Inventory.CountItem(GEM_FULL_VNUM) < gem[Upgrade])
                        {
                            session.SendPacket(session.Character.GenerateSay(
                                string.Format(Language.Instance.GetMessageFromKey("NOT_ENOUGH_ITEMS"), ServerManager.Instance.GetItem(GEM_FULL_VNUM).Name, gem[Upgrade]), 10));
                            return;
                        }

                        session.Character.Inventory.RemoveItemAmount(GEM_FULL_VNUM, gem[Upgrade]);
                    }

                    if (protection == UpgradeProtection.Protected && !isCommand)
                    {
                        session.Character.Inventory.RemoveItemAmount(GOLD_SCROLL_VNUM);
                        session.SendPacket("shop_end 2");
                    }

                    session.Character.Gold -= (long)(goldprice[Upgrade] * REDUCEDPRICEFACTOR);
                    session.Character.Inventory.RemoveItemAmount(CELLA_VNUM, (int)(cella[Upgrade] * REDUCEDPRICEFACTOR));
                    session.SendPacket(session.Character.GenerateGold());
                    break;
                ...
                case UpgradeMode.Normal:
                    ...
                    // TODO: Normal Item Amount
                    if (session.Character.Inventory.CountItem(CELLA_VNUM) < cella[Upgrade])
                    {
                        return;
                    }
                    ...
                    if (protection == UpgradeProtection.Protected && !isCommand && session.Character.Inventory.CountItem(NORMAL_SCROLL_VNUM) < 1)
                    {
                        session.SendPacket(session.Character.GenerateSay(
                            string.Format(Language.Instance.GetMessageFromKey("NOT_ENOUGH_ITEMS"), ServerManager.Instance.GetItem(NORMAL_SCROLL_VNUM).Name, 1), 10));
                        return;
                    }
                    ...
                    if (protection == UpgradeProtection.Protected && !isCommand)
                    {
                        session.Character.Inventory.RemoveItemAmount(NORMAL_SCROLL_VNUM);
                        session.SendPacket("shop_end 2");
                    }

                    session.Character.Inventory.RemoveItemAmount(CELLA_VNUM, cella[Upgrade]);
                    session.Character.Gold -= goldprice[Upgrade];
                    session.SendPacket(session.Character.GenerateGold());
                    break;
            }
