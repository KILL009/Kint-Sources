        ...
        public override void Use(ClientSession session, ref ItemInstance inv, byte option = 0, string[] packetsplit = null)
        {
            switch (Effect)
            {
                case 0:
                    if (option == 0)
                    {
                        if (packetsplit != null && packetsplit.Length == 9)
                        {
                            ...
                            if (box != null)
                            {
                                ...
                                else if (box.HoldingVNum == 0 && box.SpLevel != 1)
                                {
                    ...
                    else
                    {
                        ...
                        if (box != null)
                        {
                            ...
                            {
                                var heldMonster = ServerManager.Instance.GetNpc(box.HoldingVNum);
                                if (heldMonster != null)
                                {
                                    var mate = new Mate(session.Character, heldMonster, box.SpLevel, MateType.Pet)
                                    {
                                        Attack = box.SpDamage,
                ...
                case 1:
                    ...
                    {
                        var heldMonster = ServerManager.Instance.GetNpc((short)EffectValue);
                        if (session.CurrentMapInstance == session.Character.Miniland && heldMonster != null)
                        {
                            var mate = new Mate(session.Character, heldMonster, LevelMinimum, ItemSubType == 1 ? MateType.Partner : MateType.Pet);
                            if (session.Character.AddPet(mate))
                            {
                                session.Character.Inventory.RemoveItemAmountFromInventory(1, inv.Id);
                                session.SendPacket(UserInterfaceHelper.Instance.GenerateInfo(Language.Instance.GetMessageFromKey("PET_LEAVE_BEAD")));
