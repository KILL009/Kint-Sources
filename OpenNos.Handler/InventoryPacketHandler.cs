...
namespace OpenNos.Handler
{
    public class InventoryPacketHandler : IPacketHandler
    {
        ...
        public void Deposit(DepositPacket depositPacket)
        {
            ...
            var item = Session.Character.Inventory.LoadBySlotAndType(depositPacket.Slot, depositPacket.Inventory);
            var itemdest = Session.Character.Inventory.LoadBySlotAndType(depositPacket.NewSlot, depositPacket.PartnerBackpack ? InventoryType.PetWarehouse : InventoryType.Warehouse);
            ...
            switch (equipmentInfoPacket.Type)
            {
                ...
                case 5:
                    if (Session.Character.ExchangeInfo != null)
                    {
                        ...
                        {
                            var id = exch.ExchangeList.ElementAt(equipmentInfoPacket.Slot).Id;
                ...
                case 6:
                    if (equipmentInfoPacket.ShopOwnerId != null)
                    {
                        ...
                        var item = shop.Value?.Items.FirstOrDefault(i => i.ShopSlot.Equals(equipmentInfoPacket.Slot));
                        if (item != null)
                        {
                                ...
                                lock (Session.Character.Inventory)
                                {
                                    ...
                                    if (Session.Character.ExchangeInfo.Validate && targetExchange.Validate)
                                    {
                                        ...
                                        if (targetExchange.Confirm && Session.Character.ExchangeInfo.Confirm)
                                        {
                                            ...
                                            if (!@continue || goldmax)
                                            {
                                                var message = !@continue ? UserInterfaceHelper.Instance.GenerateMsg(Language.Instance.GetMessageFromKey("NOT_ENOUGH_PLACE"), 0)
        ...
        public void GetItem(GetPacket getPacket)
        {
            ...
            {
                var button = Session.CurrentMapInstance.Buttons.FirstOrDefault(s => s.MapButtonId == getPacket.TransportId);
                if (button != null)
                {
            ...
            else
            {
                ...
                switch (getPacket.PickerType)
                {
                    ...
                    case 2:
                        var mate = Session.Character.Mates.FirstOrDefault(s => s.MateTransportId == getPacket.PickerId && s.CanPickUp);
                        if (mate != null)
                        {
                ...
                if (mapItem is MonsterMapItem item)
                {
                    ...
                    {
                        var group = ServerManager.Instance.Groups.FirstOrDefault(g => g.IsMemberOfGroup(monsterMapItem.OwnerId.Value) && g.IsMemberOfGroup(Session.Character.CharacterId));
                        var mate = Session.Character.Mates.Find(s => s.MateTransportId == monsterMapItem.OwnerId);
                        if (item.CreatedDate.AddSeconds(30) > DateTime.Now && !(monsterMapItem.OwnerId == Session.Character.CharacterId || mate == null || @group != null && @group.SharingMode == (byte)GroupSharingType.Everyone))
                        {
        ...
        public void PutItem(PutPacket putPacket)
        {
            lock (Session.Character.Inventory)
            {
                var invitem = Session.Character.Inventory.LoadBySlotAndType(putPacket.Slot, putPacket.InventoryType);
                if (invitem != null && invitem.Item.IsDroppable && invitem.Item.IsTradable && !Session.Character.InExchangeOrTrade && putPacket.InventoryType != InventoryType.Bazaar)
                {
                    if (putPacket.Amount > 0 && putPacket.Amount < 100)
                    {
                        ...
                        {
                            var droppedItem = Session.CurrentMapInstance.PutItem(putPacket.InventoryType, putPacket.Slot, putPacket.Amount, ref invitem, Session);
                            if (droppedItem == null)
                            {
        ...
        public void Remove(RemovePacket removePacket)
        {
            ...
            var inv = Session.Character.Inventory.MoveInInventory(removePacket.InventorySlot, equipment, InventoryType.Equipment);
        ...
        public void SpTransform(SpTransformPacket spTransformPacket)
        {
            ...
            if (spTransformPacket.Type == 10)
            {
                ...
                var slElement = CharacterHelper.Instance.SlPoint(specialistInstance.SlElement, 2);
                var slHp = CharacterHelper.Instance.SlPoint(specialistInstance.SlHP, 3);
                var slDefence = CharacterHelper.Instance.SlPoint(specialistInstance.SlDefence, 1);
                var slHit = CharacterHelper.Instance.SlPoint(specialistInstance.SlDamage, 0);
            ...
            else if (!Session.Character.IsSitting)
            {
                ...
                else
                {
                    ...
                    if (timeSpanSinceLastSpUsage >= Session.Character.SpCooldown)
                    {
                        ...
                        {
                            var delay = DateTime.Now.AddSeconds(-6);
            ...
            switch (uptype)
            {
                ...
                case 3:
                    ...
                    if (inventory != null)
                    {
                        ...
                        {
                            var cellon = Session.Character.Inventory.LoadBySlotAndType(upgradePacket.CellonSlot.Value, upgradePacket.CellonInventoryType.Value);
                            if (cellon == null)
                            {
                                // ERROR IN PACKET
                                return;
                            ...
                            if (cellon.Item.Effect == 100)
                            {
                                ...
                                // GENERATE OPTION
                                var option = CellonGeneratorHelper.Instance.GenerateOption(cellon.Item.EffectValue);
        ...
        public void Wear(WearPacket wearPacket)
        {
            ...
            var inv = Session.Character.Inventory.LoadBySlotAndType(wearPacket.InventorySlot, InventoryType.Equipment);
            if (inv?.Item == null)
            {
                return;
            }
        ...
        {
            var previousInventory = Session.Character.Inventory.LoadBySlotAndType(withdrawPacket.Slot, withdrawPacket.PetBackpack ? InventoryType.PetWarehouse : InventoryType.Warehouse);
        ...
        private static void Exchange(ClientSession sourceSession, ClientSession targetSession)
        {
            ...
            targetSession.Account.BankGold += (sourceSession.Character.ExchangeInfo.BankGold*1000);
