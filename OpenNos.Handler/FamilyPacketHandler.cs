...
namespace OpenNos.Handler
{
    public class FamilyPacketHandler : IPacketHandler
    {
        ...
        public void CreateFamily(CreateFamilyPacket createFamilyPacket)
        {
            ...
            DAOFactory.FamilyDAO.InsertOrUpdate(ref familyDTO);

            Family family = Family.FromDTO(familyDTO);
            family.FamilyCharacters = new List<FamilyCharacter>();
            family.Warehouse = new Inventory(Session.Character);
            family.FamilyLogs = new List<FamilyLogDTO>();

            ServerManager.Instance.FamilyList.Add(family);
            ...
            foreach (ClientSession session in Session.Character.Group.Characters)
            {
                ...
                family.FamilyCharacters.Add(FamilyCharacter.FromDTO(familyCharacter));
                session.Character.Family = family;
            }

            ServerManager.Instance.FamilyRefresh(familyDTO.FamilyId);
            CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage
            {
                DestinationCharacterId = familyDTO.FamilyId,
                SourceCharacterId = Session.Character.CharacterId,
                SourceWorldId = ServerManager.Instance.WorldId,
                Message = "fhis_stc",
                Type = MessageType.Family
        ...
        public void FamilyDeposit(FDepositPacket fDepositPacket)
        {
            ...
            var item = Session.Character.Inventory.LoadBySlotAndType(fDepositPacket.Slot, fDepositPacket.Inventory);
            var itemdest = Session.Character.Family.Warehouse.LoadBySlotAndType(fDepositPacket.NewSlot, InventoryType.FamilyWareHouse);
        ...
        public void FamilyKick(string packet)
        {
            ...
            var kickSession = ServerManager.Instance.GetSessionByCharacterName(packetsplit[2]);
            if (kickSession != null && kickSession.Character.Family?.FamilyId == Session.Character.Family.FamilyId)
            {
                ...
                var dbCharacter = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name.Equals(packetsplit[2]) && s.State == (byte)CharacterState.Active);
                if (dbCharacter == null)
                {
                    return;
                }

                var dbFamilyCharacter = DAOFactory.FamilyCharacterDAO.FirstOrDefault(s => s.CharacterId.Equals(dbCharacter.CharacterId));
        ...
        public void FamilyManagement(FamilyManagementPacket familyManagementPacket)
        {
            ...
            switch (familyManagementPacket.FamilyAuthorityType)
            {
                case FamilyAuthority.Head:
                    ...
                    var chara = targetSession.Character.FamilyCharacter;
        ...
        public void FamilyWithdraw(FWithdrawPacket fWithdrawPacket)
        {
            ...
            var previousInventory = Session.Character.Family.Warehouse.LoadBySlotAndType(fWithdrawPacket.Slot, InventoryType.FamilyWareHouse);
            ...
            {
                var fhead = Session.Character.Family.FamilyCharacters.FirstOrDefault(s => s.Authority == FamilyAuthority.Head);
                if (fhead == null)
                {
                    return;
                }
        ...
        public void InviteFamily(string packet)
        {
            ...
            var otherSession = ServerManager.Instance.GetSessionByCharacterName(packetsplit[2]);
        ...
        public void JoinFamily(JoinFamilyPacket joinFamilyPacket)
        {
            ...
            inviteSession.Character.Family.FamilyCharacters.Add(FamilyCharacter.FromDTO(familyCharacter));
        ...
        public void TodayMessage(string packet)
        {
            ...
            var islog = Session.Character.Family.FamilyLogs.Any(s => s.FamilyLogType == FamilyLogType.DailyMessage && s.FamilyLogData.StartsWith(Session.Character.Name) && s.Timestamp.AddDays(1) > DateTime.Now);
            if (!islog)
            {
