...
namespace OpenNos.Handler
{
    public class CommandPacketHandler : IPacketHandler
    {
        ...
        {
            var player = ServerManager.Instance.GetSessionByCharacterName(act4ConnectPacket.Name ?? Session.Character.Name);
            if (player == null)
            {
                return;
            }
        ...
        public void AddMonster(AddMonsterPacket addMonsterPacket)
        {
            ...
            {
                Session.SendPacket(UserInterfaceHelper.Instance.GenerateMsg(Language.Instance.GetMessageFromKey("CANNOT_IN_MINILAND"), 0));
                return;
            }

            var npcmonster = ServerManager.Instance.GetNpc(addMonsterPacket.MonsterVNum);
            ...
            if (DAOFactory.MapMonsterDAO.FirstOrDefault(s => s.MapMonsterId == monster.MapMonsterId) == null)
            {
                DAOFactory.MapMonsterDAO.InsertOrUpdate(ref monster);

                if (DAOFactory.MapMonsterDAO.FirstOrDefault(s => s.MapMonsterId == monsterClone.MapMonsterId) is MapMonster mapMonster)
                {
        ...
        public void AddPet(AddPetPacket addPetPacket)
        {
            if (addPetPacket != null)
            {
                ...
                if (addPetPacket.Level < 1)
                {
                    addPetPacket.Level = 1;
                }

                var monster = ServerManager.Instance.GetNpc(addPetPacket.MonsterVNum);
                if (monster != null)
                {
                    var mate = new Mate(Session.Character, monster, addPetPacket.Level, MateType.Pet)
                    {
                        Attack = 0,
                        Defence = 0
                    };
            ...
            {
                Session.SendPacket(Session.Character.GenerateSay(AddPetPacket.ReturnHelp(), 10));
            }
        }
        ...
        public void AddSkill(AddSkillPacket addSkillPacket)
        {
            ...
            {
                var skillVNum = addSkillPacket.SkillVnum;
                var skillinfo = ServerManager.Instance.GetSkill(skillVNum);
                if (skillinfo == null)
                {
                ...
                else
                {
                    ...
                    {
                        var oldupgrade = Session.Character.Skills.Select(s => s.Value).FirstOrDefault(s =>
        ...
        public void Ban(BanPacket banPacket)
        {
            if (banPacket != null)
            {
                ...
                var character = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == banPacket.CharacterName && s.State == (byte)CharacterState.Active);
                if (character != null)
                {
        ...
        public void BlockExp(BlockExpPacket blockExpPacket)
        {
            if (blockExpPacket != null)
            {
                ...
                var character = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == blockExpPacket.CharacterName && s.State == (byte)CharacterState.Active);
                if (character != null)
                {
                    LogHelper.Instance.InsertCommandLog(Session.Character.CharacterId, blockExpPacket, Session.IpAddress);
                    var session = ServerManager.Instance.Sessions.FirstOrDefault(s => s.Character?.Name == blockExpPacket.CharacterName);
        ...
        public void BlockFExp(BlockFExpPacket blockFExpPacket)
        {
            if (blockFExpPacket != null)
            {
                ...
                var character = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == blockFExpPacket.CharacterName && s.State == (byte)CharacterState.Active);
                if (character != null)
                {
                    LogHelper.Instance.InsertCommandLog(Session.Character.CharacterId, blockFExpPacket, Session.IpAddress);
                    var session = ServerManager.Instance.Sessions.FirstOrDefault(s => s.Character?.Name == blockFExpPacket.CharacterName);
        ...
        public void BlockRep(BlockRepPacket blockRepPacket)
        {
            if (blockRepPacket != null)
            {
                ...
                var character = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == blockRepPacket.CharacterName && s.State == (byte)CharacterState.Active);
                if (character != null)
                {
                    LogHelper.Instance.InsertCommandLog(Session.Character.CharacterId, blockRepPacket, Session.IpAddress);
                    var session = ServerManager.Instance.Sessions.FirstOrDefault(s => s.Character?.Name == blockRepPacket.CharacterName);
        ...
        public void CharStat(CharacterStatsPacket characterStatsPacket)
        {
            ...
            if (characterStatsPacket != null)
            {
                ...
                if (int.TryParse(characterStatsPacket.CharacterName, out int sessionId))
                {
                    ...
                    {
                        var character = ServerManager.Instance.GetSessionBySessionId(sessionId).Character;
                        SendStats(character);
                    }
                    else
                    {
                ...
                else if (!string.IsNullOrEmpty(name))
                {
                    ...
                    var characterDto = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == name && s.State == (byte)CharacterState.Active);
        ...
        public void Command(HelpPacket helpPacket)
        {
            ...
            // Sort
            cmds = cmds.OrderBy(x => x).ToList();

            // Print
            foreach (string cmd in cmds)
                Session.SendPacket(Session.Character.GenerateSay(cmd, 12));
        ...
        public void CreateItem(CreateItemPacket createItemPacket)
        {
            if (createItemPacket != null)
            {
                ...
                var iteminfo = ServerManager.Instance.GetItem(vnum);
                if (iteminfo != null)
                {
                    ...
                    var inv = Session.Character.Inventory.AddNewToInventory(vnum, amount, Rare: rare, Upgrade: upgrade, Design: design).FirstOrDefault();
                    if (inv != null)
                    {
        ...
        public void Demote(DemotePacket demotePacket)
        {
            ...
            var character = DAOFactory.CharacterDAO.FirstOrDefault(s => s.State == (byte)CharacterState.Active && s.Name == demotePacket.CharacterName);
            ...
            var account = DAOFactory.AccountDAO.FirstOrDefault(s => s.AccountId == character.AccountId);
            ...
            {
                Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("LOWEST_AUTHORITY_REACHED"), 10));
                return;
            }
            ...
            var session = ServerManager.Instance.Sessions.FirstOrDefault(s => s.Character?.Name == character.Name);

            if (session != null)
            {
                session.Account.Authority = session.Character.Authority = account.Authority;
                ServerManager.Instance.ChangeMap(session.Character.CharacterId);
            }
        ...
        public void Gift(GiftPacket giftPacket)
        {
            if (giftPacket != null)
            {
                ...
                {
                    var chara = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == giftPacket.CharacterName && s.State == (byte)CharacterState.Active);
                    if (chara != null)
                    {
        ...
        public void KickSession(KickSessionPacket kickSessionPacket)
        {
            if (kickSessionPacket != null)
            {
                ...
                Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("DONE"), 10));
                var account = DAOFactory.AccountDAO.FirstOrDefault(s => s.Name == kickSessionPacket.AccountName);
        ...
        {
            var pet = Session?.Character?.Mates?.Where(x => x.IsTeamMember && x.MateType == MateType.Pet).FirstOrDefault();

            if (pet == null)
            {
                Session?.SendPacket("msg 0 " + Language.Instance.GetMessageFromKey("NO_PET"));
                return;
            }
        ...
        public void Position(PositionPacket positionPacket)
        {
            ...
            Session.SendPacket(Session.Character.GenerateSay(
                $"Map:{Session.CurrentMapInstance.Map.MapId} - X:{Session.Character.PositionX} - Y:{Session.Character.PositionY} - Dir:{Session.Character.Direction} - Grid:{Session.CurrentMapInstance.Map.Grid[Session.Character.PositionX, Session.Character.PositionY].Value} IsPVP:{ Session.Character.MapInstance.IsPVP}",
                12));
        }
        ...
        public void Promote(PromotePacket promotePacket)
        {
            ...
            var character = DAOFactory.CharacterDAO.FirstOrDefault(s => s.State == (byte)CharacterState.Active && s.Name == promotePacket.CharacterName);
            ...
            if (account.Authority >= Enum.GetValues(typeof(AuthorityType)).Cast<AuthorityType>().Max())
            {
                Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("HIGHEST_AUTHORITY_REACHED"), 10));
                return;
            }
        ...
        public void RemoveMob(RemoveMobPacket removeMobPacket)
        {
            ...
            var monst = Session.CurrentMapInstance.GetMonster(Session.Character.LastMonsterId);
            if (monst != null)
            {
                if (monst.IsAlive)
                {
                    ...
                    var monster = DAOFactory.MapMonsterDAO.FirstOrDefault(s => s.MapMonsterId.Equals(monst.MapMonsterId));
        ...
        public void RemovePortal(RemovePortalPacket removePortalPacket)
        {
            ...
            var portal = Session.CurrentMapInstance.Portals.FirstOrDefault(s =>
        ...
        public void Summon(SummonPacket summonPacket)
        {
            if (summonPacket != null)
            {
                ...
                var npcmonster = ServerManager.Instance.GetNpc(summonPacket.NpcMonsterVNum);
                if (npcmonster == null)
                {
                    return;
                }
                ...
                for (int i = 0; i < summonPacket.Amount; i++)
                {
                    ...
                    // TODO: Find a fancy way to parallelize as we dont care about order it needs to
                    //       be randomized
        ...
        public void SummonNPC(SummonNPCPacket summonNpcPacket)
        {
            ...
            if (summonNpcPacket != null)
            {
                ...
                var npcmonster = ServerManager.Instance.GetNpc(summonNpcPacket.NpcMonsterVNum);
                if (npcmonster == null)
                {
                    return;
                }
        ...
        public void TeleportToMe(TeleportToMePacket teleportToMePacket)
        {
            ...
            if (teleportToMePacket != null)
            {
                ...
                {
                    var targetSession = ServerManager.Instance.GetSessionByCharacterName(teleportToMePacket.CharacterName);
        ...
        public void Unban(UnbanPacket unbanPacket)
        {
            if (unbanPacket != null)
            {
                ...
                var chara = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == name && s.State == (byte)CharacterState.Active);
                if (chara != null)
                {
                    var log = ServerManager.Instance.PenaltyLogs.FirstOrDefault(s => s.AccountId == chara.AccountId && s.Penalty == PenaltyType.Banned && s.DateEnd > DateTime.Now);
                    if (log != null)
                    {
            ...
            if (unmutePacket != null)
            {
                ...
                if (chara != null)
                {
                    if (ServerManager.Instance.PenaltyLogs.Any(s => s.AccountId == chara.AccountId && s.Penalty == (byte)PenaltyType.Muted && s.DateEnd > DateTime.Now))
                    {
                        ...
                        var log = ServerManager.Instance.PenaltyLogs.FirstOrDefault(s => s.AccountId == chara.AccountId && s.Penalty == (byte)PenaltyType.Muted && s.DateEnd > DateTime.Now);
                        if (log != null)
                        {
                            log.DateEnd = DateTime.Now.AddSeconds(-1);
                            Session.Character.InsertOrUpdatePenalty(log);
        ...
        public void Warn(WarningPacket warningPacket)
        {
            ...
            var character = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == characterName && s.State == (byte)CharacterState.Active);
            if (character != null)
            {
                ...
                Session.Character.InsertOrUpdatePenalty(log);
                var penaltyCount = DAOFactory.PenaltyLogDAO.Where(s => s.AccountId == character.AccountId).Count(p => p.Penalty == PenaltyType.Warning);
                switch (penaltyCount)
                {
                    case 2:
        ...
        {
            var mateNpc = ServerManager.Instance.GetNpc(vnum);
        ...
        {
            var characterToMute = DAOFactory.CharacterDAO.FirstOrDefault(s => s.Name == characterName && s.State == (byte)CharacterState.Active);
        ...
        private void SendStats(CharacterDTO character)
        {
            ...
            var session = ServerManager.Instance.GetSessionByCharacterName(character.Name);
            if (session == null)
            {
                return;
            }
