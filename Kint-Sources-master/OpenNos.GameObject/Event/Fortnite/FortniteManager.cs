...
namespace OpenNos.GameObject.Event.Fortnite
{
    public class FortniteManager : Core.Utilities.Singleton<FortniteManager>
    {
        #region Members
        ...
        public async void Prepare(bool useTimer)
        {
            ...
            ServerManager.Instance.Broadcast(
                UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("FORTNITE_OPEN"), 0));
            IsLocked = false;
            ...
            {
                session.SendPacket(UserInterfaceHelper.GenerateInfo(_introductionInfo));
                session.SendPacket("gb 3 0 0 0");
                session.SendPacket($"s_memo 6 {Language.Instance.GetMessageFromKey("FORTNITE_BANK_MEMO")}");
            }
                ...
                for (int i = minutesTo - 1; i > 0; i--)
                {
                    Observable.Timer(TimeSpan.FromMinutes(i)).Subscribe(s =>
                    {
                        ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(
                            string.Format(Language.Instance.GetMessageFromKey("FORTNITE_REGISTRATION"), i), 0));
                    });
                }
            }
        }
        ...
        public async void Start()
        {
            ...
            if (_mapInstance.Sessions.Count() < 3)
            {
                _mapInstance.Broadcast(
                    UserInterfaceHelper.GenerateMsg(
                        Language.Instance.GetMessageFromKey("FORTNITE_NOT_ENOUGH_PLAYERS"), 0));
            ...
            _mapInstance.Broadcast(
                UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("FORTNITE_STARTED"),
                    0));
        ...
        public async void Stop()
        {
            ...
            void NotifyEnd()
            {
                ...
                _clientSessions.ForEach(s => { s.SendPacket("say 1 0 10 [FORTNITE] There is no winner !"); });
            }
        }
        ...
        public void Kick(ClientSession kickedSession, ClientSession killerSession = null)
        {
            ...
            _mapInstance.Broadcast(UserInterfaceHelper.GenerateMsg(
                string.Format(Language.Instance.GetMessageFromKey("FT_KILL"), kickedSession?.Character?.Name,
